    ...
    <div class="row">
        <div class="col-12">
                    ...
                    <div class="row align-items-center">
                        <div class="col">
                            <h5 class="mb-0">
                                ...
                                <span class="badge bg-light text-dark ms-2">{{ $viagens->total() }}</span>
                            </h5>
                        </div>
                        <div class="col-auto">
                            <a href="{{ route('viagens.create') }}" class="btn btn-success">
                    ...
                    <form method="GET" action="" class="row g-3 align-items-end">
                        ...
                        <div class="col-12 col-sm-6 col-lg-3">
                            ...
                            <select name="tipo_veiculo" id="filtro_tipo_veiculo" class="form-select">
                                ...
                                <option value="Carro" {{ request('tipo_veiculo') == 'Carro' ? 'selected' : '' }}>Carro</option>
                                <option value="Moto" {{ request('tipo_veiculo') == 'Moto' ? 'selected' : '' }}>Moto</option>
                                <option value="Caminhão" {{ request('tipo_veiculo') == 'Caminhão' ? 'selected' : '' }}>Caminhão</option>
                            </select>
                        </div>
                        ...
                        <div class="col-12 col-lg-8">
                            <div class="d-flex align-items-start">
                                ...
                                <div class="flex-grow-1">
                                    <h6 class="mb-2 fw-bold text-primary fs-5">{{ $viagem->destino }}</h6>
                                    <div class="text-muted">
                                        <div class="row g-2">
                                            <div class="col-12 col-sm-6">
                                                <i class="bi bi-calendar me-1 text-primary"></i>
                                                <strong>{{ \Carbon\Carbon::parse($viagem->data)->format('d/m/Y') }}</strong>
                                            </div>
                                            <div class="col-12 col-sm-6">
                                                <i class="bi bi-person me-1 text-info"></i>
                                                <strong>{{ $viagem->condutor }}</strong>
                                            </div>
                                            @if($viagem->tipo_veiculo)
                                            <div class="col-12 col-sm-6">
                                                <i class="bi bi-car-front me-1 text-success"></i>{{ $viagem->tipo_veiculo }}
                                                @if($viagem->placa)
                                                    <span class="badge bg-secondary ms-2">{{ $viagem->placa }}</span>
                                                @endif
                                            </div>
                                            @endif
                                            <div class="col-12 col-sm-6">
                                                <i class="bi bi-clock me-1 text-warning"></i>
                                                <strong>{{ $viagem->hora_saida }}</strong>
                                                @if($viagem->hora_chegada)
                                                    <i class="bi bi-arrow-right mx-1"></i><strong>{{ $viagem->hora_chegada }}</strong>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="mt-3">
                                        @if($viagem->km_saida && $viagem->km_chegada)
                                            @php
                                                $distancia = $viagem->km_chegada - $viagem->km_saida;
                                            @endphp
                                            <span class="badge bg-success fs-6 px-3 py-2">
                                                <i class="bi bi-check-circle me-2"></i>Concluída - {{ $distancia }}km
                                            </span>
                                        @elseif($viagem->km_saida)
            ...
            <div class="d-flex justify-content-center mt-4">
                {{ $viagens->appends(request()->query())->links('pagination::bootstrap-4') }}
            </div>
            @endif
        </div>
    </div>
